...
export class WorkflowDatabase {
  ...
  static getTrainingSessions(): TrainingSession[] {
    const stmt = getDB().prepare("SELECT * FROM training_sessions ORDER BY started_at DESC")
  ...
  static updateTrainingSession(id: number, updates: Partial<TrainingSession>): void {
    const fields = Object.keys(updates).map((key) => `${key} = ?`).join(", ")
    const values = Object.values(updates)
    const stmt = getDB().prepare(`UPDATE training_sessions SET ${fields} WHERE id = ?`)
    stmt.run(...values, id)
  }
  ...
  static getPredictions(): Prediction[] {
    const stmt = getDB().prepare("SELECT * FROM predictions ORDER BY created_at DESC")
  ...
  static updatePrediction(id: number, updates: Partial<Prediction>): void {
    const fields = Object.keys(updates).map((key) => `${key} = ?`).join(", ")
    const values = Object.values(updates)
    const stmt = getDB().prepare(`UPDATE predictions SET ${fields} WHERE id = ?`)
    stmt.run(...values, id)
  }
  ...
  static getDatasets(): Dataset[] {
    const stmt = getDB().prepare("SELECT * FROM datasets ORDER BY created_at DESC")
  ...
  static getRecentActivity(limit = 50): ActivityLog[] {
    const stmt = getDB().prepare("SELECT * FROM activity_logs ORDER BY timestamp DESC LIMIT ?")
